...
## 4. Deductions — imported as data, not yet applied
...
### Status: imported and tested, not yet wired into taxable income
...
Replacing it is a separate change, because the interesting part is not the
arithmetic but deciding **which rules apply to a given household**: the export
lists every deduction a canton offers, and a taxpayer takes a subset depending on
marital status, children, employment, and property. Summing all 574 would be as
wrong as the current estimate, in the opposite direction.
